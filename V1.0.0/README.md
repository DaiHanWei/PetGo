# TailTopia · Vet-Care Design Tokens (BMAD Phase 1)

A **parallel** Phase-1 token set extracted from the Figma file
**“Vet Care – veterinary care app UI (Community)”** (`YU083N0uVpVVTS6k4rVxB2`, light mode, mobile 430 px).

This set is **independent** of the existing `tokens/tokens.css` (blue *Paw Buddy*) that currently
drives the 29-page app. Nothing in `src/` imports this folder — adopt it deliberately.
...
- **Mono-violet brand.** Primary `#7D16FF` (Log In button, active category). Soft companion
  `#A259FF` (cards), light `#B77EFF` (borders/circles), tint `#F8F2FF` (canvas). No second hue.
...
- ✅ Primary color defined — `--color-brand-primary` = `#7D16FF`; secondary = `#A259FF`.
...
```css
/* re-theme the whole app: swap the import in src/index.css */
@import '../tokens/vet-care/tokens.css';   /* instead of '../tokens/tokens.css' */
```
...
Semantic var names match the blue set, so this is a drop-in re-theme. Most pairings pass AA; the
ones that don't are **faithful to the Figma source** and are listed here so you use them correctly.
Targets: normal text **4.5:1**, large/bold text & UI **3:1**.

| Pairing | Ratio | Verdict | Guidance |
|---------|------:|---------|----------|
| text-primary (`#544864`) on bg-default/surface | 7.7 / 8.4 | ✅ AA | default body & headings |
| white on **brand-primary** (`#7D16FF`) — buttons | 6.1 | ✅ AA | primary CTA, any size |
| link (`#7D16FF`) on bg-default | 5.6 | ✅ AA | — |
| info (`#530BA6`) / success / warning / error **-text** on their `-subtle` | 4.8–8.3 | ✅ AA | use `--color-*-text` for badge labels |
| white on **brand-secondary** (`#A259FF`, violet-400 cards) | 3.88 | ⚠️ large-only | **≥18.66px bold / ≥24px or icons only** (matches Figma card labels); for small text use brand-primary |
| **text-secondary** (`#808080`) on light bg | 3.6–4.0 | ⚠️ large-only | for small AA body, override to `var(--grey-700)` `#545454` (~7:1) |
| **text-tertiary** (`#A7A7A7`) on light bg | 2.2–2.4 | ⛔ < 3:1 | **placeholder / timestamp / hint only — never content.** Need readable tertiary text? use `--grey-700` |
| **border-brand** (`#B77EFF`) on bg-default | 2.6 | ⛔ < 3:1 | **decorative borders only.** Sole-indicator/interactive bounds → `--color-border-focus` (violet-500, passes) |

**Dark theme:** brand-primary is held at violet-500 so white CTA text stays **6.1:1**; text-primary
**16.6:1**, text-secondary **8.8:1**, link **6.7:1** — all AA. Status chips stay tonal (own bg + AA
text) and remain legible on dark. To keep small **secondary/tertiary** text AA-safe by default across
the app, remap `--color-text-secondary` → `--grey-700` and `--color-text-tertiary` → `--grey-700`.
