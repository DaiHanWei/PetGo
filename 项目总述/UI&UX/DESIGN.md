...
- **Refund choice sheet** — inherits `{components.sheet}`. Two full-width option rows stacked (not side-by-side, unlike the paywall sheet), each styled as `{components.button-secondary}` at rest; once one path is chosen, the unchosen option is removed from view rather than disabled-grayed (avoids an ambiguous disabled state on a financial choice).
- **已暂停 banner** — inline banner card, `{rounded.md}`, `{colors.status-warning-subtle}` background, `{colors.status-warning}` left accent or icon (never `status-error` — the order is paused, not failed). Persists at the top of the order detail view, not a dismissible toast.
...
- **Monthly recap card** — `{components.card-lg}` with the same brand-gradient treatment as the AI-consultation entry card (`linear-gradient` `{colors.brand-secondary}` → `{colors.brand-primary}`), signaling "generated/special" rather than a plain content card. Share action uses `{components.button-primary}` styling.
